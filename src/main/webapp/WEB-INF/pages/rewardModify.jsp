...
<script>
    ...
    const app = Vue.createApp({
        ...
        methods: {
            btnSubmit(){
                //检查输入
                if(this.uid.trim().length==0||this.reason.trim().length==0|| this.type.trim().length==0 || this.time.trim().length==0){
                    window.parent.swalForWarning("提示", "请先完善奖惩信息！");
                    return;
                }
                //请求后端
                axios.post("/api/reward/modify.do", {
                    rid: this.rid,
                    uid: this.uid,
                    reason: this.reason,
                    type: this.type,
                    ...
                    if(JSON.parse(response.data.status)){
                        //新增成功
                        window.parent.swalForSuccess("提示","修改奖惩信息成功！");
                        //跳转修改页面
                        window.parent.showPage("rewardManage");
                    }else{
                        //新增失败
                        window.parent.swalForError("提示", "修改奖惩信息失败，可能是当前奖惩信息不存在！");
                    }
                }).catch((error) => {
                    // 弹出错误信息
                    window.parent.swalForError('提示','网络请求失败，请稍后重试！');
