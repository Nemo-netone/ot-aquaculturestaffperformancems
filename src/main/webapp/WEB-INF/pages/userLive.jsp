...
<script>
            ...
            axios.post("/api/live/list.do", {
                ...
                if(JSON.parse(response.data.status)){
                    ...
                    for(var i = 0 ; i<list.length;i++){
                        tbodyHtml += '<tr data-id="' + list[i].lid + '">';
                        tbodyHtml += '<td>' + (i+1) + '</td>';
    ...
    function doModify(_this){
        //获取ID
        var lid=$(_this).parent().parent().attr("data-id");
        //跳转修改页面
        window.parent.showPage("/liveModify?lid="+lid);
    }
    //删除信息
