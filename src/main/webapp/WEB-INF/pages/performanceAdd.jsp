        ...
        <div class="card">
            ...
            <div class="card-body">
                <form id="app">
                    ...
                    <div class="form-group">
                        <label>考核周期(开始)</label>
                        <input type="date" class="form-control" v-model="period1" placeholder="请输入考核周期" autocomplete="off">
                    </div>
                    <div class="form-group">
                        <label>考核周期(截止)</label>
                        <input type="date" class="form-control" v-model="period2" placeholder="请输入考核周期" autocomplete="off">
                    </div>
...
<script>
    ...
    const app = Vue.createApp({
        ...
        methods: {
            btnSubmit(){
                //检查输入
                if(this.uid.trim().length==0 || this.period1.trim().length==0 || this.period2.trim().length==0  ){
                    window.parent.swalForWarning("提示", "请先完善绩效考核信息！");
                    return;
                }
                //请求后端
                axios.post("/api/performance/add.do", {
                    uid: this.uid,
                    period: this.period1+"至"+this.period2,
                }).then((response) => {
                    // 判断结果
                    if(JSON.parse(response.data.status)){
                        //新增成功
                        window.parent.swalForSuccess("提示","新增绩效考核信息成功！");
                        //刷新页面
                        window.location.reload();
                    } else {
                        //新增失败
                        window.parent.swalForError("提示", "新增绩效考核信息失败，可能是当前绩效考核信息已存在！");
                    }
                }).catch((error) => {
                    // 弹出错误信息
                    window.parent.swalForError('提示','网络请求失败，请稍后重试！');
