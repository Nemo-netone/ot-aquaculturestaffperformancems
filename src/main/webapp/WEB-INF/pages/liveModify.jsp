            ...
            <div class="card-body">
                <form id="app">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" v-model="data" placeholder="请输入日增重(kg/只)" autocomplete="off">
                    </div>
                    <div class="form-group">
                        <label>动物物种</label>
...
<script>
    ...
    const app = Vue.createApp({
        ...
        methods: {
            btnSubmit(){
                //检查输入
                if(this.data.trim().length==0|| this.remarks.trim().length==0 || this.operator.trim().length==0){
                    window.parent.swalForWarning("提示", "请先完善生长指标！");
                    return;
                }
                //请求后端
                axios.post("/api/live/modify.do", {
                    lid: this.lid,
                    data: this.data,
                    ...
                    if(JSON.parse(response.data.status)){
                        //新增成功
                        window.parent.swalForSuccess("提示","修改生长指标成功！");
                        //跳转修改页面
                        window.parent.showPage("liveManage");
                    }else{
                        //新增失败
                        window.parent.swalForError("提示", "修改生长指标失败，可能是当前生长指标不存在！");
                    }
                }).catch((error) => {
                    // 弹出错误信息
                    window.parent.swalForError('提示','网络请求失败，请稍后重试！');
