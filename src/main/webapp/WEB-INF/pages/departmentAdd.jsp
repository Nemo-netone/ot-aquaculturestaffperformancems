...
<script>
    ...
    const app = Vue.createApp({
        ...
        methods: {
            btnSubmit(){
                //检查输入
                if(this.dname.trim().length==0 || this.description.trim().length==0 ){
                    window.parent.swalForWarning("提示", "请先完善部门信息！");
                    return;
                }
                //请求后端
                axios.post("/api/department/add.do", {
                    dname: this.dname,
                    ...
                    if(JSON.parse(response.data.status)){
                        //新增成功
                        window.parent.swalForSuccess("提示","新增部门信息成功！");
                        //刷新页面
                        window.location.reload();
                    } else {
                        //新增失败
                        window.parent.swalForError("提示", "新增部门信息失败，可能是当前部门信息已存在！");
                    }
                }).catch((error) => {
                    // 弹出错误信息
