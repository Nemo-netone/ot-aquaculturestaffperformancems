...
<script>
    ...
    const app = Vue.createApp({
        ...
        mounted(){
            //请求后端加载数据
            axios.post("/api/task/listByuid.do", {
                ...
                if(JSON.parse(response.data.status)){
                    ...
                    for(var i = 0 ; i<list.length;i++){
                        tbodyHtml += '<tr data-id="' + list[i].tid + '">';
                        tbodyHtml += '<td>' + (i+1) + '</td>';
                        tbodyHtml += '<td><pre>' + list[i].description + '</pre></td>';
                        tbodyHtml += '<td>' + list[i].stime + '</td>';
                        tbodyHtml += '<td>' + list[i].etime + '</td>';
                        tbodyHtml += '<td>' + list[i].status + '</td>';
                        tbodyHtml += '<td><pre>' + list[i].completion + '</pre></td>';
                        tbodyHtml += '<td><img src="img/' + list[i].photo + '" style="height: 60px;width: 60px" alt="未完成" /></td>';
                        tbodyHtml += '<td>' + list[i].operator + '</td>';
                        if(list[i].status=="未完成"){
                            tbodyHtml += '<td><button type="button" class="btn btn-info" onclick="doModify(this)">提交任务</button>';
                        }else{
                            tbodyHtml += '<td><button type="button"  class="btn btn-secondary" style="cursor: not-allowed;">已完成</button>';
                        }
                        tbodyHtml += '</td></tr>';
                    }
                    //展示数据
    ...
    function doModify(_this){
        //获取ID
        var tid=$(_this).parent().parent().attr("data-id");
        window.parent.showPage("/userTaskMod?tid="+tid);
    }
</script>
</body>
</html>
