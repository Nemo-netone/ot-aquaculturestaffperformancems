            ...
            <div class="card-body">
                <form id="app">
                    <div class="form-group">
                        <label>部门名称</label>
                        <input type="text" class="form-control" v-model="dname" placeholder="请输入部门名称" autocomplete="off">
                    </div>
                    <div class="form-group">
                        <label>部门描述</label>
                        <input type="text" class="form-control" v-model="description" placeholder="请输入部门描述" autocomplete="off">
                    </div>
...
<script>
    ...
    const app = Vue.createApp({
        ...
        mounted(){
            //获取信息
            axios.post("/api/department/get.do", {
                did: this.did
            }).then((response) => {
                // 判断结果
                if(JSON.parse(response.data.status)){
                    //显示信息
                    this.dname=response.data.data.dname;
        ...
        methods: {
            btnSubmit(){
                //检查输入
                if(this.dname.trim().length==0|| this.description.trim().length==0 || this.ctime.trim().length==0|| this.operator.trim().length==0){
                    window.parent.swalForWarning("提示", "请先完善部门信息！");
                    return;
                }
                //请求后端
                axios.post("/api/department/modify.do", {
                    did: this.did,
                    dname: this.dname,
                    ...
                    if(JSON.parse(response.data.status)){
                        //新增成功
                        window.parent.swalForSuccess("提示","修改部门信息成功！");
                        //跳转修改页面
                        window.parent.showPage("departmentManage");
                    }else{
                        //新增失败
                        window.parent.swalForError("提示", "修改部门信息失败，可能是当前部门信息不存在！");
                    }
                }).catch((error) => {
                    // 弹出错误信息
                    window.parent.swalForError('提示','网络请求失败，请稍后重试！');
