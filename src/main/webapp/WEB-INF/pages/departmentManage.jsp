...
<script>
            ...
            axios.post("/api/department/list.do", {
                ...
                if(JSON.parse(response.data.status)){
                    ...
                    for(var i = 0 ; i<list.length;i++){
                        tbodyHtml += '<tr data-id="' + list[i].did + '">';
                        tbodyHtml += '<td>' + (i+1) + '</td>';
                        tbodyHtml += '<td>' + list[i].dname + '</td>';
    ...
    function doModify(_this){
        //获取ID
        var did=$(_this).parent().parent().attr("data-id");
        //跳转修改页面
        window.parent.showPage("departmentModify?did="+did);
    }
    //删除信息
    function doDelete(_this){
        ...
        var did=$(_this).parent().parent().attr("data-id");
        //询问是否确认删除
        window.parent.swalForQuestion("提示","请确认是否删除该部门信息?",function(){
            axios.post("/api/department/delete.do", {
                did:did
            }).then((response) => {
                // 判断结果
                if(JSON.parse(response.data.status)){
                    //删除成功
                    window.parent.swalForSuccess("提示","删除部门信息成功！");
                    //刷新页面
                    window.location.reload();
                } else {
                    //删除失败
                    window.parent.swalForError("提示","删除部门信息失败，请检查部门信息是否存在!");
                }
            }).catch((error) => {
                // 弹出错误信息
                window.parent.swalForError('提示','网络请求失败，请稍后重试！');
