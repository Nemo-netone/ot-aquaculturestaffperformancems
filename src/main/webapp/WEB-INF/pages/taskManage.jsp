...
<script>
    ...
    function doModify(_this){
        ...
        //跳转修改页面
        window.parent.showPage("/taskModify?tid="+tid);
    }
    //删除信息
    function doDelete(_this){
        ...
        var tid=$(_this).parent().parent().attr("data-id");
        //询问是否确认删除
        window.parent.swalForQuestion("提示","请确认是否删除该任务信息?",function(){
            axios.post("/api/task/delete.do", {
                tid:tid
            }).then((response) => {
                // 判断结果
                if(JSON.parse(response.data.status)){
                    //删除成功
                    window.parent.swalForSuccess("提示","删除任务信息成功！",function(){});
                    //刷新页面
                    window.location.reload();
                } else {
                    //删除失败
                    window.parent.swalForError("提示","删除任务信息失败，请检查任务信息是否存在!");
                }
            }).catch((error) => {
                // 弹出错误信息
                window.parent.swalForError('提示','网络请求失败，请稍后重试！');
