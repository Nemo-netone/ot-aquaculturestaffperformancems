            ...
            <div class="card-body">
                <form id="app">
                    <div class="form-group">
                        <label>部门名称</label>
                        <select class="form-control" v-model="did" v-html="departmentHtml">
...
<script>
    ...
    const app = Vue.createApp({
        ...
        methods: {
            btnSubmit(){
                //检查输入
                if(this.pname.trim().length==0 || this.description.trim().length==0 || this.requirements.trim().length==0){
                    window.parent.swalForWarning("提示", "请先完善岗位信息！");
                    return;
                }
                //请求后端
                axios.post("/api/position/add.do", {
                    did: this.did,
                    pname: this.pname,
                    ...
                    if(JSON.parse(response.data.status)){
                        //新增成功
                        window.parent.swalForSuccess("提示","新增岗位信息成功！");
                        //刷新页面
                        window.location.reload();
                    } else {
                        //新增失败
                        window.parent.swalForError("提示", "新增岗位信息失败，可能是当前岗位信息已存在！");
                    }
                }).catch((error) => {
                    // 弹出错误信息
                    window.parent.swalForError('提示','网络请求失败，请稍后重试！');
