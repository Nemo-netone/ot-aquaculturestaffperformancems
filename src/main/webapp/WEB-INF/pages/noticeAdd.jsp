        ...
        <div class="card">
            ...
            <div class="card-body">
                <form id="app">
                    ...
                    <button type="button" class="btn btn-primary btn-lg mt-3" @click="btnSubmit">发布</button>
                </form>
            </div>
        </div>
    </div>
...
<script>
    ...
    const app = Vue.createApp({
        ...
        methods: {
            btnSubmit(){
                //检查输入
                if(this.title.trim().length==0 || this.notice.trim().length==0 ){
                    window.parent.swalForWarning("提示", "请先完善公告信息！");
                    return;
                }
                //请求后端
                axios.post("/api/notice/add.do", {
                    title: this.title,
                    notice: this.notice,
                }).then((response) => {
                    // 判断结果
                    if(JSON.parse(response.data.status)){
                        //新增成功
                        window.parent.swalForSuccess("提示","新增公告信息成功！");
                        //刷新页面
                        window.location.reload();
                    } else {
                        //新增失败
                        window.parent.swalForError("提示", "新增公告信息失败，可能是当前公告信息已存在！");
                    }
                }).catch((error) => {
                    // 弹出错误信息
