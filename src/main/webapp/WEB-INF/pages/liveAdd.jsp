...
<script>
    ...
    const app = Vue.createApp({
        ...
        methods: {
            btnSubmit(){
                //检查输入
                if( this.data.trim().length==0 || this.remarks.trim().length==0){
                    window.parent.swalForWarning("提示", "请先完善生长指标！");
                    return;
                }
                //请求后端
                axios.post("/api/live/add.do", {
                    data: this.data,
                    ...
                    if(JSON.parse(response.data.status)){
                        //新增成功
                        window.parent.swalForSuccess("提示","新增生长指标成功！");
                        //刷新页面
                        window.location.reload();
                    } else {
                        //新增失败
                        window.parent.swalForError("提示", "新增生长指标失败，可能是当前生长指标已存在！");
                    }
                }).catch((error) => {
                    // 弹出错误信息
