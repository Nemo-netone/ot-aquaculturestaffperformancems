...
<script>
            ...
            axios.post("/api/user/list.do", {
                ...
                if(JSON.parse(response.data.status)){
                    ...
                    for(var i = 0 ; i<list.length;i++){
                        tbodyHtml += '<tr data-id="' + list[i].uid + '">';
                        tbodyHtml += '<td>' + list[i].uid+'</td>';
                        tbodyHtml += '<td>' + list[i].department.dname+'</td>';
                        tbodyHtml += '<td>' + list[i].position.pname+'</td>';
                        tbodyHtml += '<td>' + list[i].username+'</td>';
                        tbodyHtml += '<td>' + list[i].identify+'</td>';
                        tbodyHtml += '<td>' + list[i].name+'</td>';
                        tbodyHtml += '<td>' + list[i].sex+'</td>';
                        tbodyHtml += '<td>' + list[i].age+'</td>';
                        tbodyHtml += '<td>' + list[i].birthdate+'</td>';
                        tbodyHtml += '<td>' + list[i].idcard+'</td>';
                        tbodyHtml += '<td>' + list[i].phone+'</td>';
                        tbodyHtml += '<td><img src="img/' + list[i].avatar+'" style="width: 50px;height: 50px" /></td>';
    ...
    function doModify(_this){
        //获取ID
        var uid=$(_this).parent().parent().attr("data-id");
        //跳转修改页面
        window.parent.showPage("userModify?uid="+uid);
    }
    //删除信息
    function doDelete(_this){
        ...
        var uid=$(_this).parent().parent().attr("data-id");
        //询问是否确认删除
        window.parent.swalForQuestion("提示","请确认是否删除该员工信息?",function(){
            axios.post("/api/user/delete.do", {
                uid:uid
            }).then((response) => {
                // 判断结果
                if(JSON.parse(response.data.status)){
                    //删除成功
                    window.parent.swalForSuccess("提示","删除员工信息成功！");
                    //刷新页面
                    window.location.reload();
                } else {
                    //删除失败
                    window.parent.swalForError("提示","删除员工信息失败，请检查员工信息是否存在!");
                }
            }).catch((error) => {
                // 弹出错误信息
                window.parent.swalForError('提示','网络请求失败，请稍后重试！');
