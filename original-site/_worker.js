...
export default {
  async fetch(request, env) {
    try {
      ...
      const routedPage = await routeOriginalPage(request, env, url);
...
async function routeOriginalPage(request, env, url) {
  if (!env.ASSETS || url.pathname.startsWith("/api/") || url.pathname === "/health") return null;
  if (url.pathname === "/") return fetchAsset(request, env, "/login");
  if (url.pathname === "/index") return fetchAsset(request, env, "/dashboard");
  if (url.pathname === "/userLogin" || url.pathname === "/login") {
