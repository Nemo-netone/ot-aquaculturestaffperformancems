			...
			if(isShowImg==false){
				...
				if($(".faceDiv").children().length==0){
					for(var i=0;i<ImgIputHandler.facePath.length;i++){
						$(".faceDiv").append("<img title=\""+ImgIputHandler.facePath[i].faceName+"\" src=\"http://www.sucai8.cn/static/i/face/"+ImgIputHandler.facePath[i].facePath+"\" />");
					}
...
$(function() {
    $('#_fix_btn').on('click', function() {
        ...
        if(kw) {
            window.location.href="http://www.sucai8.cn/search/"+kw;
        }
    });
});
